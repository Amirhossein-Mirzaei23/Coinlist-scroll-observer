  ...
  {{ props.isConected }}
        <tbody >  
          ...
          <!----use update price function to upadate coin percent with websocket data the open_24 value of api fetch is same with websocjet----->
            <td class="flex flex-row-reverse gap-2 justify-end" ><span ><span>{{ ( updatePrice(coinData) ) }}</span>{{coinData.data}}</span> <span v-if="coinData.symbol.search('USDT') > 0" >USDT</span> <span v-else >IR</span></td>
            <td class="w-6/12 m-auto flex flex-row-reverse gap-1 justify-center badge bg-green-600 bg-opacity-10 text-green-500 "  :class="{ 'bg-red-700': updatePrice(coinData) < coinData.open_24h,'text-red-600':updatePrice(coinData) < coinData.open_24h }" ><span></span><span>%</span> {{ updatePercent(coinData) }} </td>
            <td>
              <div class="flex flex-row gap-2 justify-end" >
                <figure  class="text-lg" >{{ coinData.symbol.slice(0,-5) }} </figure>
                <img class="w-8 h-8 rounded-full" src="~/assets/image/btc.png" alt="">
              </div>
            </td>
...
function observerHandeler(observedCoins) { 
  ...
  console.log("call");
   observerHandeler(observerdViewportCoinList)
    ...
    watch(()=>props.isConected, async (newQuestion, oldQuestion) => {
   if (data.value) {
    console.log("watch");
      callWebsocket()
   }
...
function updatePrice(coin) {
  let coinLastPrice = coin.last
    if (coin) {
      
      if (props.recivedWebsocketData[0]) {
        //match target coin with recived data props
        props.recivedWebsocketData.map((x) => {
          x.arg.instId == coin.symbol ?
          coinLastPrice = x.data[0].last
           : null
        })
        return coinLastPrice
      } else {    
        // return price which recived data from the request
        return coinLastPrice
      }
    }
}
...
function updatePercent(coin) {
  ...
  let coinLastPercent = ((Math.abs((Math.round((((coin.last - coin.open_24h) / coin.open_24h) * 10000))) / 100)))
      ...
      if (props.recivedWebsocketData[0]) {
        ...
        props.recivedWebsocketData.forEach((x) => {
          ...
          if(x.arg.instId == coin.symbol){
            coinLastPercent =((Math.abs( (Math.round((((x.data[0].last -   x.data[0].open24h )/  x.data[0].open24h)*10000)))/100)))
          }
        })
